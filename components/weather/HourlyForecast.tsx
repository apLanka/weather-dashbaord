...
export function HourlyForecast() {
  ...
  return (
    <Card className="p-6 bg-slate-800/90 backdrop-blur-sm border-slate-700 shadow-xl">
      <h3 className="text-lg font-semibold text-slate-100 mb-4">Hourly Forecast</h3>
      ...
      <ScrollArea className="w-full">
        <div className="flex gap-4 pb-4">
          {hourlyData.map((hour, index) => (
            ...
            >
              <div className="text-sm text-slate-300 mb-2 font-medium">{hour.time}</div>
              <div className="flex justify-center mb-2">
                <WeatherIcon condition={hour.condition} size="small" />
              </div>
              <div className="text-slate-100 font-semibold mb-1">{hour.temperature}°</div>
